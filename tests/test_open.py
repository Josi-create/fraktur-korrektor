...
def test_vorschlag_nach_erscheinungsjahr(lib, tmp_path):
    ...
    assert lib.lget('/buch/%s/api/settings' % r['id'])[1] == dict(year=2010, dics=['1901', 'neu'], notizen=None, kennung=None, autor=None)
...
def test_vorschlag_wird_fuer_aeltere_importe_nachgeholt(tmp_path):
    ...
    try:
        assert server.Book(str(book)).settings == dict(year=2010, dics=['1901', 'neu'], notizen=None, kennung=None, autor=None)
        assert json.load(open(book / 'buch.json', encoding='utf-8'))['year'] == 2010
        assert server.Book(str(fremd)).settings == dict(year=None, dics=['1901'], notizen=None, kennung=None, autor=None) and not (fremd / 'buch.json').exists()  # von Hand angelegt: bleibt
    finally:
